...
class NinjaFormQueryCreateView(APIView):
    authentication_classes = []  # No authentication required
    permission_classes = []      # No permissions required
    
    def generate_email(self, first_name, last_name, phone):
        """Generate email from name and phone if email not provided"""
        # Clean phone number and use last 4 digits
        clean_phone = re.sub(r'\D', '', phone)[-4:]
        # Create email using firstname, lastname and last 4 digits of phone
        email = f"{first_name.lower()}.{last_name.lower()}{clean_phone}@generated.vitigo.com"
        return email
    ...
    def post(self, request):
        try:
            ...
            # Extract data from Ninja Forms submission
            ...
            may_contact = data.get('may_contact', False)
            best_time = data.get('best_time', '')

            # If no email provided but phone exists, generate email
            if not email and phone:
                email = self.generate_email(first_name, last_name, phone)
            ...
            query = Query.objects.create(
                user=user,
                subject=f"Website Enquiry from {first_name} {last_name}",
                description=f"""
Enquiry Details: {enquiry_details}
Location: {place}, {country}
Preferred Center: {center}
May Contact: {'Yes' if may_contact else 'No'}
Best Time to Call: {best_time}
                """.strip(),
                source='WEBSITE',
                priority='B',  # Medium priority
                status='NEW',
                is_anonymous=False if user else True,
                contact_email=email,
                contact_phone=phone,
                query_type='GENERAL',
                is_patient=True
